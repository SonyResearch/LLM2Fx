...
def load_effects_encoder():
    ...
    """
    ckpt_path = os.path.join("ckpt", "fxenc", 'fxenc_default.pt')
    with open(os.path.join("ckpt", "fxenc",'configs.yaml'), 'r') as f:
        configs = yaml.full_load(f)
    cfg_enc = configs['Effects_Encoder']['default']
    effects_encoder = Effects_Encoder(cfg_enc)
    reload_weights(effects_encoder, ckpt_path)
    return effects_encoder

def reload_weights(model, ckpt_path, device="cpu"):
...
class Effects_Encoder(nn.Module):
    ...
    @torch.no_grad()
    def get_global_embedding(self, input):
        enc_output = self.forward(input)
        glob_pooled = enc_output.mean(dim=2) # b,d,l -> b,d
        return glob_pooled
